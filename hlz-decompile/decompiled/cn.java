...
extends ec {
    ...
    public cn(short s, short s2, short s3, byte by) {
        super(s, s2, s3, by, b.a[ck.f].a);
        s = 1;
        switch (s3) {
            case 102: 
            case 125: {
    ...
    protected boolean a(int n) {
        if (this.i == (byte)(11 + this.f)) {
            ...
            return n >= v && n <= w && n % 2 == 0;
        }
        return false;
    }

    protected void e() {
        int n = ((c)this).c + this.ac[this.f << 1];
        int n2 = ((c)this).c + this.ac[(this.f << 1) + 1];
        do[] doArray = db.a();
        for (int i = 0; i < 2; ++i) {
            if (doArray[i] == null || doArray[i].c < n || doArray[i].c > n2) continue;
